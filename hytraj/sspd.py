...
class TrajCluster:
    ...
    def plot_cluster(self):
        fig, ax = facets(1, 1, width=10, aspect=0.4, internal_pad=0.3)
        ax = ax[0]
    ...
    def get_basemap(self, ax=None, min_lat=-35):
        if not ax:
            ...
            ax = ax[0]
        m = Basemap(
            projection="spstere", lon_0=180, boundinglat=min_lat, round=True, ax=ax
        )
    ...
    def plot_color_trajectories(
        self, ax=None, cmap=plt.cm.jet, lw=0.3, s=200, min_lat=-35, c="r"
    ):
        m = self.get_basemap(ax=ax, min_lat=min_lat)
        xx, yy = m(self.slon, self.slat)
    ...
    def plot_clustered_trajectories(
        self, ax=None, cmap=plt.cm.jet, lw=3, s=200, min_lat=-35, scale=10, c="r"
    ):
    ...
    def plot_representative_trajectories(
        self, ax=None, cmap=plt.cm.jet, lw=3, s=200, min_lat=-35
    ):
        m = self.get_basemap(ax=ax, min_lat=min_lat)
        xx, yy = m(self.slon, self.slat)
        m.scatter(xx, yy, color="r", s=s)
    ...
    def plot_dendrogram(self, ax=None, p=10, D=50):
        if not ax:
            fig, ax = facets(1, 1, width=10, aspect=0.5)
            ax = ax[0]
        dn = sch.dendrogram(
            self.link,
            ax=ax,
...
class Trajectory:
    def __init__(
        ...
        height="5000",
        runtime=360,
        base="/mnt/repo4/hysplit/antarctic/gdas/",
        ddir="./data/gdas/",
    ):
    ...
    def save(self):
        ...
        self.trajs = py.TrajectoryGroup(
            [traj for traj in trajg if len(traj.data.index) == self.runtime + 1]
        )
